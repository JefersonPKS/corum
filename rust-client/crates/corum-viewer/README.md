...
### Interface original e modo jogo (2026-09-19)

- **Como abrir para jogar:** `.\jogar.ps1` (ou `Jogar.bat`), com o guia `COMO_JOGAR.md` na raiz de `rust-client`. O lançador usa o build de release, define `CORUM_GAME=1` e monta o personagem vestido (`-Classe`, `-Mapa`, `-Arma`...).
- **Modo jogo (`CORUM_GAME=1`):** o mouse anda (clique) e gira a câmera (arrastar); `WASD` deixa de andar e as **teclas originais** (`KeyConfig.ini`) abrem as janelas: `T` inventário (`ITEM`), `A` personagem (`CHAR`), `S` habilidades (`SKILL`), `O` opções (`GAMEMENU`); `Esc` fecha a janela da frente e, sem nenhuma, sai. Sem `CORUM_GAME` (modo de desenvolvimento) o `WASD` anda e as janelas abrem com `I`, `C`, `K`, `P`. `CORUM_UI=off` desliga a interface.
- **O que a interface é hoje:** o desenho **estático** das janelas originais, lido das tabelas (`InterfaceFrameInfo`, `InterfaceComponentInfo`, `InterfaceSpriteManager`, `InterfaceResourceInfo`) e das imagens do pacote `UI`: 84 janelas conhecidas, quatro delas com atalho. Tela de referência 1024 × 768, escalada por igual e centralizada na janela (`ui_gpu::layout`). As janelas abrem na posição da tabela, vêm para a frente ao clicar, **arrastam pela barra de título** (áreas `CHECK_MOVEWND`) e **fecham pelo "X"** (`CHECK_CLOSE`); um clique dentro de uma janela não vira ordem de andar. **Sem dados nem texto** (nome, PV, itens): as tabelas não os trazem e ainda não há fonte de texto; os botões ainda não fazem nada; a barra principal (PV/PM, atalhos, minimapa, chat) é desenhada por código no original e não foi refeita.
- **Código:** `corum_assets::ui` (catálogo e `UiDesktop`, com a lógica de janelas testada), `corum_assets::tga` (decodificador dos `.tga`), e no viewer `src/ui_gpu.rs` + `src/ui_shader.wgsl` (passada 2D sem profundidade, uma textura por atlas criada na primeira vez que a janela abre).
- **Orientação das imagens:** os `.tga` da interface (24 bits, origem embaixo) são invertidos ao decodificar; os `.tif` da interface saem certos **sem** a inversão que os materiais do mapa precisam (a inversão do mapa é da convenção de UV).
- **Mapas de teste:** `604` (padrão do lançador), `1100`, `5` e `750` abrem completos; `10001` (mapa de mundo) abre, mas sem relevo (o `.hfl` não foi decifrado). O `.stm` é lido de `Map_stm.pak` quando não está ao lado do `.ttb`.
- **Frente dos modelos [confirmado na tela]:** o sandbox gira o ator pelo rumo do movimento, supondo que a frente do modelo é +Z. Os modelos do pacote `Character` (jogador e suas peças) olham para o lado contrário e andavam de costas; `facing_offset` gira 180° os modelos desse pacote (a cabeça, o elmo, a arma e o escudo acompanham o corpo). Monstros e NPCs ficam com deslocamento 0 e **a frente deles não foi verificada** (o teste com o monstro como jogador ficou inconclusivo).
